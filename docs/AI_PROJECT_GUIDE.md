# 鲸语 WhaleTalk 项目全览 · AI 开发速查手册（v3.16.7）
...
- **版本单一源**：`config_defaults.py` 的 `VERSION`（当前 `3.16.7`）。
- **能力规模（`tools/check_docs.py` 实测口径，2026-09）**：**165 个 Agent 工具**（11 组）、
  **101 个 `/v1` 路由**、**82 个 pytest 文件 / 835 用例 + 20 个前端 node 套件**、
...
# ── 测试（改完代码必跑）──
python -m pytest -q                      # 后端回归（82 个文件 / 835 用例；CI 同款）
...
| `tests/` | 后端回归 82 个 pytest 文件 / 835 用例 |
...
## 14. 测试资产速查（tests/，82 个 pytest 文件 / 835 用例）
...
*本文档由 AI 读取源码后整理，符号名与代码一致；规模数字（165 工具 / 101 路由 / 835 用例 / 版本 3.16.7）
