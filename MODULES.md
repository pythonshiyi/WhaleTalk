...
> 规模口径（`tools/check_docs.py` 实测）：**162 个 Agent 工具**（11 组）· **99 /v1 端点** · 后端 78 个 pytest 文件 / 803 用例 · 前端 14 个 node 套件；源码约 6.2 万行（根目录 2.9 万 + `agent_tools/` 1.7 万 + `webui/src` 1.6 万）。
...
| `tests/` | 自举回归套件（78 个 pytest 文件 / 803 用例）：注册表六层一致性、域模块拆分 re-export、进化闸、失败生命周期、上下文装配、退化日志、出网账本、记忆门面、信任内核、大脑、安全/权限/网络等；`self_evolve` 验证链自动回退跑全量 |
