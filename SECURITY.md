...
- **run_python（无沙箱，但有资源护栏）**：等同本机 `python -c` 直通解释器——无 `-I -S` 隔离、无静态 AST 危险检查；能力与风险均由用户显式授权承担。资源护栏仅作**防误伤兜底**（非沙箱、非策略限制）：同步超时 `RUN_PY_TIMEOUT=60s`、输出截断 `RUN_PY_MAX_OUTPUT=20000 字符`、进程树内存看门狗 `RUN_PY_MEMORY_MB=2048MB`（psutil 轮询 RSS，超限杀树并如实报错，避免一次失控分配拖垮用户整机）；更重的任务应走 `start_process` 后台通道
