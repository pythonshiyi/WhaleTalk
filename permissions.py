...
**唯一的程序内置底线**：网络侧的 SSRF 硬底线——私网段（10/172.16/192.168）、
链路本地（169.254.0.0/16，含全部云元数据地址）、保留段一律拦截（含 DNS 重绑定
校验）；回环默认放行。它不依赖用户配置（可在 network 中单独关闭），因为
「模型可自主抓取任意 URL + 抓取内容回灌上下文」构成 prompt injection 面，
仅靠用户黑名单无法覆盖。其余限制全部来自用户配置。
"""
import json
import logging
import os
...
DEFAULT_PERMISSIONS = {
    ...
    "network": {
        ...
        # SSRF 硬底线（security._hard_floor_reason）：私网段（10/172.16/192.168）、
        # 链路本地（169.254.0.0/16，含全部云元数据地址）、保留段一律拦截，
        # 且域名会先做 DNS 解析——默认模式下同样防 DNS 重绑定。默认开。
        "block_private": True,
        # 回环（localhost / 127.0.0.1）是否放行：默认放行——本机单用户软件，
        # 本地开发服务器验证是高频正当场景（工具层默认本就允许本机任意执行）。
        # 需加严（例如担心被诱导访问本机 API）置 false。
        "allow_loopback": True,
    },
