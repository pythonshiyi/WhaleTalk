...
- **规模**：165 工具（11 组）· 101 个 `/v1` 路由 · 后端 82 个 pytest 文件 / 836 用例 · 前端 20 个 node 套件。
...
- **版本单一源**：`config_defaults.VERSION`（当前 **3.16.8**），最新变更见 [CHANGELOG.md](CHANGELOG.md)。
...
**WhaleTalk v3.16.8** is a local-first Windows AI desktop agent built around the unified **DeepSeek V4.1 Flash** model (`deepseek-flash`, natively multimodal). It runs as a local API (`127.0.0.1:8745`) with a React 19 / Vite 8 web UI and a system-tray resident process — the browser is the only window.
