...
### 聊天：工具按发生顺序交错（修正 v3.16.14）

- 新增纯函数 `buildTurnFlow(text, segs, tools)`：依据分段锚点 `segs`（该段正文之前已完成几步），把工具**切组插到正确的段间位置**——`正文① → 执行过程(1–4步) → ↳基于第4步 → 正文② → 执行过程(第5步)`，步骤号全局连续。
- 步骤组**默认展开**（直接看到每一步），可逐组收起；「查看全部」链接只在最后一组出现一次。无锚点的历史会话退化为「先步骤、后正文」。

### 右栏：新增「交付物」（按类型 / 最新优先）

- **数据**：新增 `GET /v1/deliverables`；跨会话读取 `recent_outputs.json`，**规范化路径 + 大小写去重**，剔除**系统路径**（`C:\Windows` 等）与**目录**，补齐类型/大小/时间，按时间倒序。
- **后端清洗** `_record_recent_output`：只收真实文件、跳过系统路径/目录、规范化去重（修复把 AI 脚本、字体、工作区目录、`d:\\`/`D:\\` 重复项一并记录的噪声问题）。
- **前端**：与「本会话产物」合并去重 → **按类型分组**（视频/图片/文档/表格/演示/音频/数据/压缩/代码/其他），可切「最新优先 / 按时间（刚刚·今天·更早）」；**代码/脚本默认折叠**（一键显示）；支持搜索、类型筛选；每项 打开 / 定位 / 复制路径 / 注入输入框，最新项带「最新」标记。

### 验证

`pytest` **851 passed / 1 skipped**；前端 `npm run typecheck`、`npm test`、`npm run build` 全绿；`tools/check_docs.py` 通过（`/v1` 路由 101→102）。

---

## v3.16.14（2026-09-21）—— 🧭 聊天信息流重构：思考 / 执行 / 交付三层可读

**版本号 3.16.13 → 3.16.14。** 把「思考、工具执行、正文交付」从混排改为**分层可读**：工具不再只剩一句摘要跳侧栏，而是内联为**过程带**；正文与步骤有因果；产物回合内直达。

### 变化（纯前端重排，不改 SSE 协议）

- **回合头状态徽标**：生成中 / ✓已完成·N 步 / ⚠含失败 / 生成中断 + 时长。
- **思考块**：生成中自动展开+脉冲，结束自动收起并显示体量（x 字）。
- **过程带（`ProcessRail`）**：默认折叠为「N 步 · X 失败 · 共 Ts」+ 进度条；展开为时间线——工具家族图标 / 名称 / 参数摘要 / 状态节点（等待·执行中·完成·失败分色）/ 耗时；点步骤展开**参数与结果**、可复制；底部直达右栏「活动」审计。
- **计划（`PlanBlock`）**：`plan_request` 落地为 checklist，状态由已执行工具推导。
- **正文因果**：多段输出按**分段锚点**在段间插入「↳ 基于第 N 步结果」。
- **产物**：回合底部内联（打开/定位），仅从**产出型工具结果 + 正文**提取，排除只读工具避免把输入当产物。
- 保留：指标条、操作条、用户附件回显、错误块（真实错误 + 重试）；右栏「活动」原样保留为全局审计。

### 数据与兼容

- 新增可选字段 `segs`（分段锚点 `{i,n}`）：前端记录、后端**校验后持久化并在重载时还原**；旧会话无此字段时优雅降级。不改任何 SSE 事件与请求契约。

### 工程

- 新增纯函数 `webui/src/segmentNotes.js`（分段切分）+ `tests/segmentNotes.test.mjs`；`Message.jsx` 重写为回合卡，`ChatPage.jsx` 记录锚点/计划，`api_server.py` 持久化并回读 `segs`，`app.css` 增补样式与三档密度。

### 验证

`pytest` **851 passed / 1 skipped**；前端 `npm run typecheck`、`npm test`（新增分段用例）、`npm run build` 全绿；`tools/check_docs.py` 通过。
...
- **前端显示真实错误**（`ChatPage.jsx`）：错误块改为 `生成中断：<后端真实信息>`，仅当后端未给出详情时才回退到原通用文案。
- **后端落盘排障日志**（`api_server.py`）：对话失败时把**异常类型 + 原文 + traceback**追加到 `data/logs/chat_errors.log`。托盘/无窗启动时 stdout/stderr 不可见，此后可事后诊断。
- `_friendly_error` 补充「上下文超限」类错误的中文可操作提示。

### 背景（本次实测）

- 当前运行网关（OpenCode Go）经 12 次 task 请求与最长 ~60 万 tokens 超长上下文测试均正常，系统代理亦已关闭——故本次先解决「错误不可见」，待拿到真实错误再针对性修复。

### 验证

`pytest` **848 passed / 1 skipped**；前端 `npm run typecheck`、`npm test` 全绿；`tools/check_docs.py` 通过。

---

## v3.16.11（2026-09-21）—— 💾 会话崩溃安全：生成中途被杀不再丢会话

**版本号 3.16.10 → 3.16.11。** 回应实测：会话过程中若进程被关闭/崩溃，重启后该会话**完全没有任何记录**——因为此前只在回合结束时才落盘，新会话中途被杀就全丢。

### 根因

- 会话只在**回合结束**落盘（前端 `onFinished` 保存；无人订阅时后端 `_save_job_turn` 兜底）。生成过程中没有任何持久化。

### 修复：在制回合（turn）增量落盘

- `_save_session_locked` 新增在制回合语义（`turn_id` + `turn_len`）：支持「同一 turn_id 就地替换」，既不重复追加 user，也不堆积助手分片。
- 作业线程 `_run_chat_job_thread`：发出 `session` 事件后**立即落一版**（含本轮 user），生成期间每 ~4s 及每次工具完成再落一版（`[本轮 user] + 已流式正文`）。进程被强杀时，会话文件里已有 user 与已生成内容。
- 最终保存带同名 `turn_id` 时**就地替换**在制回合并清除标记；崩溃后继续新回合（不同 `turn_id`）时旧内容**保留**。
- 前端 `api.js` / `ChatPage.jsx`：最终保存携带本次 `stream_id` 作为 `turn_id`。

### 验证

`pytest` **848 passed / 1 skipped**（新增 `tests/test_session_inflight.py` 3 项：周期落盘就地替换 / 最终保存替换并清标记 / 崩溃后新回合保留旧在制内容）；前端 `npm run typecheck`、`npm test` 全绿；`tools/check_docs.py` 通过。
...
### 基础提示词（`DEFAULT_SYSTEM_PROMPT`）

原来只有 6 条能力声明 + 「请使用中文回答」，模型知道「能做什么」，不知道「该怎么做」：

- 新增 **输出纪律**：直接给结论/代码/操作、不寒暄不复述；解释按需；不编造 API/路径/版本号/命令；验证未通过不宣称完成；架构级决策可展开、其余保持简洁。
- 新增 **执行纪律**：动代码前先 `read_file`/`list_dir`/`project_map` 确认现状；`edit_file` 优先于 `write_file`、只改任务相关部分；覆盖/删除前确认、删除走回收站；改完走 `run_lint → run_tests → verify_project` 验证链；网络/路径/命令先过黑名单与内置底线；优先复用 `failure_memory` 已验证规避路径。
- 新增 **收尾**：只报告「改了什么文件 / 验证命令 / 结果」三件事。
- 删除写死的「100 万 Token」宣传语，改为「在长上下文中保持任务状态一致性」（上下文规模由配置决定，不再随模型版本漂移）。
- 「错误恢复」由「自主修正并继续执行」收紧为「自主定位根因并修正，不用 try/except 掩盖」。

### 压缩提示词（`_SUMMARY_PROMPT`）

- 七节 → **八节**：新增 **有效行为准则**（用户/系统对输出风格、工具使用、验证要求的约束——原样保留），避免压缩后模型丢掉说话与做事准则。
- 新增 **自检**：逐条核对「不可丢失的原文」中的约束是否都已进入上文小节，缺失补入。

### 说明

- 对话模式（`DIALOG_SYSTEM_PROMPT` / `_DIALOG_SUMMARY_PROMPT`）不涉及工具与验证，本次不改。
- 现有安装的 `config.json` 若已固化旧 `system_prompt` 会覆盖默认值；本次已同步本地 `config.json`，全新安装/恢复默认自动采用新默认。

### 验证

`pytest` 通过 · `python tools/check_docs.py` 提示词一致性门禁通过。
...
**根治**：effect 启动时对续写标记做**一次性消费**——读到即清零，并校验目标 `idx` 仍指向
一条 assistant 消息，否则一律按正常发送处理。残留标记因此无法泄漏到任何后续发送
（含新对话），从结构上杜绝「任何对话都发不出去」。

**兜底**：在**所有**本轮结束/会话切换的出口复位续写标记：
...
### 像素画笔（`image_codegen` kind=pixel）真实缺陷

v3.16.1 修的是「思考吞满预算 → content 为空」；本版修的是**生成范式层缺陷**（实测 150×190 高密度像素画必崩、自评 2/10）：

- **根因 1（提示词锁死实现方式）**：`_KIND_HINTS["pixel"]` 写死「用 SVG 小矩形拼像素」，且 `_author_prompt` 规定「一切用 CSS / 内联 SVG 绘制」→ 排除 canvas + JS 程序化绘制。高密度像素画需数万格，逐格手写受输出长度上限约束，**数学上不可达**（实测超预算百倍），模型只能退化成大色块或渐变冒充像素。
- **修复**：pixel 提示改为「网格 >32×32 必须用 `<canvas>` + `<script>` 逐像素程序化绘制（createImageData），禁止逐个手写像素块」，并加**左右镜像对称**、**先声明几何分区坐标**、**有限色板（主体 16-24 档 / 全图 32-64 档）**约束；`_author_prompt` 放行 canvas + JS（对元素密集类通用）。
- **根因 2（自评只会磨细节、不会换方案）**：低分时仅 `_revise_prompt` 在原方案上微调 → 错误方案上反复打磨仍是低分。
- **修复**：评分 ≤4 时下一轮**换方案重写**（改走 author prompt + 反例约束）；`refine_source` 若为「手写大量 `<rect>`」的旧像素方案，自动弃用并重写。
- **根因 3（缺少像素专项评审）**：评审维度不含密度 / 色数 / 平涂 / 对称。
- **修复**：`kind=pixel` 追加专项检查（密度、色数 32-64、>8×8 平涂、镜像对称），任一项不合格 score ≤4。

### `read_file` 中文乱码（独立缺陷）

- **根因**：`_detect_text_encoding` 只读前 8192 字节做严格 `decode`；若第 8192 字节正好切在 UTF-8 汉字中间，严格解码因「结尾不完整」失败 → 误判 latin-1 → 中文全乱码（同一文件 `read_project_file` 正常，两工具策略不一致）。
- **修复**：改用增量解码器 `final=False`（容忍被截断的尾部多字节，真正非法字节仍报错）。

### 附带

- 工具调用参数被 max_tokens 截断时，明确提示「参数可能被截断，请拆分内容 / 改用 write_file」，不再只报「参数非合法 JSON」。

### 验证

`pytest` **822 项**（818 passed · 4 skipped；新增 5 项回归：pixel 提示解锁 / author 放行 canvas / 评审专项 / 低分换方案 / 编码边界分割）· 四门禁全绿。
...
- `App.jsx` 条件渲染：切页即**卸载** `ChatPage`，其 cleanup 里 `abort()` 了 SSE，且
  `alive=false` 让 `finish()` 短路 → 前端从不保存本轮输出。
- 后端把生成绑在 HTTP 请求线程上：SSE 写失败即 `stop_event.set()` → 前端一断连服务端也停。
- 新会话不携带 `session_id`，后端兜底落盘被 `if sid:` 挡掉 → 保存失败。

### 修复

- **生成与连接解耦（后台作业）**：`client.chat()` 跑在独立线程，事件写入 `_ChatJob`
  内存缓冲（`_CHAT_JOBS`）；HTTP 处理器只做订阅，**掉线只解除订阅、绝不停止作业**；
  空转时写 SSE 注释帧（`: ping`）保活；晚到订阅者从缓冲起点回放。
- **仅跑完或手动停止才结束**：`POST /v1/chat/stop` 命中作业停止句柄（`stream_id` /
  `session_id` / `gw_session`）；同 `stream_id` 的重复请求订阅同一作业，不重复生成。
- **会话 id 统一**：新增首个 SSE 事件 `session`（下发后端分配的会话 id），前端据此
  落盘，杜绝「前端另存一份」与兜底落盘各生成一个 id 的重复会话。
- **断连兜底落盘**：无在线订阅者时由作业保存本轮；`DeepSeekClient.chat` 新增
  `new_messages_out` 出参，**只追加本轮**（长会话不丢历史）。
- **前端常驻挂载**：`ChatPage` 切页只 `hidden`、不卸载（流式输出与状态留存）；
  隐藏时不再响应 Ctrl+K / Esc 全局快捷键。

### 验证

`pytest` **637 passed**（新增 `tests/test_chat_jobs.py` 11 项：掉线不打断 / 停止句柄 /
重复订阅 / 兜底落盘）· `npm test` / `npm run typecheck` / `vite build` · 四道门禁全绿
（154 工具 0 error · 98 端点）。
...
- **背景**：`git pull` 更新内核文件（`permissions/security/crypto/snapshot/trust_kernel`）后，字节层面与「AI 自我修改」完全一样，信任内核会如实报警 → 每次拉取都在界面上弹出「自我完整性」待确认、并反复写 incident。正常更新被误报成篡改，噪音会淹没真异常。
- **git 锚点（自动跟随）**：`boot_check()` 先对每个「与基线不一致」的内核文件判定 `工作区内容 == git HEAD`（`git diff --quiet HEAD`，含 autocrlf 行尾归一化）。成立即判为**仓库更新**：自动推进基线（记账本事件 `repo_update`，含 HEAD 提交号；`STATUS.md` 单列一节）。**不静默**。
- **边界不放松**：与 HEAD 不一致（真实工作区改动）/ 非 git 仓库 / 文件未跟踪 → 一律照旧报警、留证。自动跟随**绝不放行自我修改**。
- **可关闭**：`trust/config.json` 的 `auto_follow_git: false` 退回严格模式（连 git 更新也报）。
- **跨启动去重**：未确认改动内容未变（同组文件 + 同当前 sha）时复用既有 incident 与证据副本，不再每次重启复制一份；内容再变才新建。
- **顺手修复**：`_record_incident`/`_keep_copy` 同名（精确到秒）不再互相覆盖（`_unique_path` 追加 `_2/_3…`）；`status(deep=True)` 漏设 `repo_updates` 抛 NameError 被吞成 unknown 的回归。
- **验证**：`pytest` 604 passed（新增 6 个 git 锚点/去重/回归用例）；真实 git 临时仓库端到端覆盖「已提交更新→自动跟随」与「未提交改动→仍报警」。

## v3.12.0（2026-09-16）—— 📎 对话附件（粘贴图片 / 拖拽文件）+ 🩹 托盘菜单卡死修复

**版本号 3.11.3 → 3.12.0。** 本版把「把材料给 AI」的门槛降到最低：图片可粘贴、文件可拖拽；并修复 v3.11.3 引入的托盘菜单卡死。

**对话附件**

- **粘贴图片**：在对话区任意位置 `Ctrl+V` 粘贴截图/图片，自动上传并作为视觉输入（普通文本粘贴不受影响）。
- **拖拽文件**：从资源管理器把文件拖到对话区出现高亮遮罩，松手即添加——图片走视觉链路，其它文件（PDF/Office/CSV/压缩包…）随消息把路径交给 AI 工具层读取。
- **多选与回显**：附件按钮支持多选；输入区显示图片缩略图/文件 chip（含大小，可移除）；发送后用户气泡回显缩略图（点开大图）与文件 chip（点击用系统程序打开）。
- **会话持久化**：图片/文件随消息写入会话（`_save_session` 新增 `images`/`files` 字段，`_load_session_messages` 回读），重载会话后仍显示缩略图与文件 chip，继续对话依然有效。
- **编辑重发保留附件**：编辑带附件的消息不再丢失原图/原文件。
- **后端**：新增 `POST /v1/files/upload`（任意文件 base64 原样落盘 `uploads/files/`，文件名消毒防目录穿越）；`/v1/upload` 继续负责图片（超限自动压缩）。

**修复**

- **托盘菜单卡死**：右键任务栏图标弹出的菜单/鼠标会卡死——v3.11.3 的 5s 刷新循环每轮 `icon.menu = …`（内部 `update_menu()`）**又**显式 `update_menu()`，在菜单展开时 `DestroyMenu` 了正在显示的原生 HMENU（pystray win32 的 `TrackPopupMenuEx` 阻塞于主循环）。现改为：**仅在菜单内容指纹变化时重建**，且**菜单展开期间（`_MENU_OPEN`）绝不触碰菜单**；开关变更经 `_wake` 立即刷新。

- **验证**：`pytest` 598 passed；`npm test` / `npm run typecheck` / `vite build` 通过。

## v3.11.3（2026-09-15）—— 🩹 补丁：托盘状态区刷新 + 版本可见性

- **修复**：托盘**状态区文本**在构建菜单时被固化，`update_menu()` 只刷新勾选态 → 定时刷新的"动态状态"实际不会更新。
  现每次刷新**重建菜单**（`icon.menu = build_menu()`）再 `update_menu()`。
- **可见性**：tray tooltip 与状态区顶部加**版本号**（`鲸语 WhaleTalk v3.11.3 · 运行中 · …`），便于确认进程是否已加载新代码。
- **说明**：托盘代码随**进程启动**加载——"重新打开"若是单实例命中，只会开浏览器、不重启服务；需先完全退出再启动。
- **验证**：`pytest` 591 passed。

## v3.11.2（2026-09-15）—— 🏷 正式发布：全站 UI 统一 · 托盘重构 · 测试隔离

**版本号 3.11.1 → 3.11.2。** 本版聚焦「体验一致性与工程健康」，把 v3.11.1 之后累计的批次一并固化发布：

| 批次 | 一句话 |
|---|---|
| 🎨 控制台风格全站统一（Wave 1+2） | 下划线页签 · 卡片/标题节拍 · 全站 SVG 图标 · 空态/会话/消息/输入区/大脑 emoji 归一 |
| 🖥 系统托盘深度优化 + 重构 | 抽 `tray.py`；配置污染修复 · 单实例互斥 · 动态状态 · 勾选态 · 常用动作 · 气泡通知 |
| 🧪 测试隔离 | 修复测试写入真实大脑记忆库（`BRAIN_DIR` 隔离）并清理既有垃圾 |

> 以下各批次条目原标注「未发版追加」，现随 v3.11.2 一并发布；条目正文保持原貌，便于追溯。

## v3.11.2（未发版追加）—— 🖥 系统托盘（任务栏图标）深度优化 + 重构

把托盘从 `web_app.py` 的静态菜单抽成独立、可测的 `tray.py`，补齐逻辑正确性、动态状态与原生交互：

- **P0 正确性**
  - 修复**共享配置污染**：旧回调 `cfg = load_config(); cfg[k]=…` 直接改进程级只读共享对象；现全部经
    `config_utils.mutable_config()`（深拷贝副本），杜绝并发读到「改了一半」的配置。
  - **单实例互斥**：新增 Windows 命名 Mutex；另一个启动器正在起服务时，本进程等待其就绪后仅打开界面退出，避免双起/端口冲突。
- **动态状态（读取前端同源数据）**：托盘 `tooltip` 与只读状态区每 5s 刷新——运行端口 / 模式 / 模型 /
  本月成本 / 累计 tokens / 高峰时段 / **内核待确认** / 降级数（数据源注入 `api_server._status`）。
- **原生交互**
  - 开关改**勾选态**（`checked=`）替代「开/关」文字；开机自启失败自动回滚 + 气泡提示。
  - 新增动作：打开**工作目录 / 数据目录 / 日志**、**清理空闲进程**、**检查更新**、**关于**。
  - **托盘气泡通知**：任务完成/失败时与 `notify_desktop` 一并弹出（复用 `notify_on_done` 开关）；
    内核由「正常→待确认」时提示一次。
  - 图标优先使用品牌 `app.ico`（缺失退回手绘兜底）。
- **架构**：`tray.py`（状态/菜单/刷新/通知/退出解耦，不 import api_server，避免导入环）；
  纯函数 `fmt_tokens/status_lines/tooltip_text/trust_unconfirmed` 可单测（`tests/test_tray.py` 7 用例）。
- **验证**：`pytest` **591 passed**；四道门禁全绿；`import web_app` / 托盘刷新链路冒烟通过。


## v3.11.2（未发版追加）—— 🎨 控制台风格全站统一（Wave 1：页签/导航/标题/卡片/图标）

把「控制台 2.0」的设计语言延伸到全站页面，消除各页各写一套的视觉漂移：

- **页签统一下划线式**：能力中心 / 自主 / 大脑 / 上下文 / 会话筛选（`.ab-tab`/`.au-tab`/`.brain-tab`/`.ctx-tab`/`.sl-tab`）
  统一为控制台的下划线页签（icon+文字，active 品牌 2px 下划线），容器统一底边线。
- **卡片与分区标题统一**：页面卡片统一圆角/边框/投影；`.wb-card-title`/`.au-card-title`/`.set-group-title`
  统一为「图标 + 标题」节拍（13px 粗体、统一间距）。
- **图标系统落地**：`icons.jsx` 扩充到 **60+** 图标；把以下 UI chrome 的 emoji 换成统一 SVG——
  设置左导航、大脑页签、自主页签与 11 张卡片标题、能力中心页签、插件页签、命令面板、
  消息操作条（复制/固定/分叉/编辑/引用/重发/朗读/收藏/继续）、确认弹窗、各页 `h1`。
- **导航气质对齐**：左侧主导航激活态统一为品牌软底。
- 验证：`npm run typecheck` + `build` + `npm test` 全绿。

> **Wave 2（本轮完成）**：`EmptyState` 支持统一图标名（新增 `hasIcon`）并归一空态图标；
> 会话列表（置顶/重命名/标签/删除/导入导出/多选）、消息操作与标记（置顶/收藏/工具摘要/任务进度）、
> 输入区（附件/工作目录菜单）、大脑指挥舱（各分区标题：指挥舱/认知时间轴/认知看板/意识状态/时光备份/生命延续）
> 的 emoji 全部换为统一 SVG。
> **有意保留**：正文/toast/提示文案里的 emoji（作为文字语气，非图标位）。
...
核心是**把「代码即设计」这条能力线补全并加装零部件**，以及一批 UI/正确性修复：

| 批次 | 一句话 |
|---|---|
| 🎨 设计/作图/排版/PDF/表格（工具 151 → 154） | 离线 Mermaid/KaTeX/ECharts + 矢量图表 + PDF 后处理套件 + 设计工具与模板 |
| 🖼 本地图片不嵌入修复 | HTML→PDF/PNG/PPT 相对图片改为内联 + `file://` 渲染 |
| 🌱 自我洞察 | 能力热力图 · 自我述职 · 信任内核故事线（工具 149） |
| 🎨 前端 UI 精修 P0→P2 | 可达性 / 令牌一致性 / 三态与无障碍 |
| 🛡 自我完整性提示 | 状态栏提示说人话 + 可点开；trust_kernel CLI 编码兜底 |
| 📋 聊天可复制 | 用户消息可选中/复制 + 复制按钮 |

> 以下各批次条目原标注「未发版追加」，现随 v3.11.1 一并发布；条目正文保持原貌，便于追溯。
...
本批次新增纯函数模块 `insight.py` + 两个 AI 工具 + 三个只读端点 + 前端「自主 → 成长」栏目。

| 特性 | 一句话 |
|---|---|
| 📊 能力热力图（工具 + 端点） | 聚合工具使用频率/任务链长度/失败率/技能结晶数/预激活命中，一眼看出「哪里用得多、哪里总出错、哪里没用过」 |
| 📋 自我述职（工具 + 端点） | 汇总一段时间内的任务/决策/目标进度/进化/技能结晶/用量，生成可写入工作目录的 Markdown 周报（可定时） |
| 🛡 信任内核故事线（端点 + 前端） | 把账本 + 未声明事件合并为统一时间轴（`trust_kernel.timeline()`），让「护栏改动史」可读、可回滚 |

### 新增

- **`insight.py`（纯函数 · 仅标准库）**：`build_heatmap` / `build_self_report` / `render_report`，
  不依赖运行时，便于单测与复用。
- **工具 `capability_heatmap(days=30)`**：能力热力图文本摘要（最常用 / 薄弱项 / 技能结晶 / 预激活命中）。
- **工具 `self_report(days=7, write=False)`**：自我述职，`write=true` 写入工作目录 `self-reports/`。
  工具数 **149 → 151**。
- **端点** `GET /v1/insight/heatmap?days=N` · `GET /v1/insight/report?days=N`；
  `GET /v1/trust?timeline=1` 追加故事线时间轴（默认不返回，避免响应膨胀）。端点 **94 → 96**。
- **`trust_kernel.timeline(limit)`**：账本（declare/commit/accept/restore…）+ `incidents/`（未声明事件）
  合并为按时间倒序的统一事件流。
- **前端「自主 → 🌱 成长」栏目**：能力热力图（带条形图与时间窗切换）+ 自我述职摘要 + 信任内核故事线。

### 设计立场

- 热力图与述职均为**只读聚合**，不改动任何既有数据；`self_report` 仅在 `write=true` 时写盘。
- 故事线**不新增写路径**：只是把既有的账本/事件读出来合并（不阻断、可回滚的立场不变）。
- 接入方式遵循既有基建：工具走 `@tool()` 六层注册；路径经 `_dc_wiring_table` 单一注入点
  （新增 `PROMPTS_FILE` / `TASKLOG_FILE`）。

### 验证

`audit_tools --strict` 151 工具 error 0 · `validate_tools` 151 全链路 · `island_check` 无孤岛 ·
`check_docs` 数字一致 · `pytest` 544 passed（+12：`test_insight.py` 8 + 信任时间轴 4）·
前端 typecheck + 29 个 node 用例 + build 全通过 · 实机端点冒烟通过。

## v3.11.0（2026-09-11）—— 🏷 正式固化 v3.10.0 之后 7 个批次 · 工具 148 → 149

**版本号 3.10.0 → 3.11.0**。本版不追能力宽度，而是把 v3.10.0 之后累计的
**7 个「未发版追加」批次一并固化发布**——补的都是已有能力的**边界与自我认知**：

| 批次 | 一句话 |
|---|---|
| 🔍 能力自省（本版核心） | 新增只读自省工具 `list_my_capabilities`（工具 148 → 149）；追问不丢预激活；预激活可度量 |
| ⇡ 边界收口 P1 | `egress.py` 出网账本（安全模型入强出弱 → 对称化）+ 记忆单一门面（血缘标注 / 作废可回溯） |
| 🧭 架构收口 P0 | 横切关注点收进钩子管线：上下文装配 · 退化日志 · 工具钩子 |
| 🛡 自我完整性 | 信任内核：自我修改**可声明 · 可见 · 可回滚** |
| 🔍 搜索健壮性 | 引擎软超时 · 翻页/站点诚实报错 · 额度预警 |
| 🗄 数据安全与进程契约 | 忽略=软删除 · 进程孤儿修复 · 契约别名 |
| 🧠 认知质量收敛 | 失败记忆生命周期 · 提案可见性 · 技能结晶 · 长任务打点 |

### 🔍 能力自省（本版要点）

- **B 新增只读自省工具 `list_my_capabilities`**：此前 AI 要确认「我到底有多少项能力」
  只能去读源码——自我认知是外挂的（上次诊断中 activate_tools 只枚举到 133 项，与声明的
  148 差 15，最终靠读 `toolkit.py` 做 AST 重建才查明）。现在一行核验 149 项 / 11 组。
  **刻意不默认全量 dump**：那会与能力地图等重（≈1.8k token），等于把按需加载省下的上下文
  又还回去——无参只给分组汇总（约百 token），再按 `group=` / `query=` 下钻；并主动解释
  「组内成员合计 150 > 总数 149」的跨组重复归属，专治「数对不上就去读源码」。
- **A 预激活扫描窗口 1 → 3**：原实现把 `return` 写在循环内，只扫最近 1 条 user 消息 →
  「帮我抓取这个网页」→「继续」，第二句不含关键词导致预激活整体失效，模型多一次
  `activate_tools` 往返。现扫最近 3 条取并集，空文本消息（纯图片）不占窗口名额。
- **C 预激活命中埋点**：86 条关键词手工维护、无命中率也无漏激活率 → 现按关键词计数 +
  尽力落盘（失败静默，埋点绝不干扰对话主流程），出口在 `usage_report`：
  「命中最多 / 从未命中 N 条（可考虑删改）」，让关键词表第一次有数据可依。

### 🧩 同步点认知修正：4 → 8 处

新增工具此前被记为「动 4 处」，实测漏了两层 re-export 闸门——只改 `@tool` +
`_TOOL_ORDER` 会让 `from agent_tools import *` 当场抛 `AttributeError`、
`deepseek_client` 导入失败。完整清单：① `@tool` → ② 领域模块 `__all__` →
③ `agent_tools/__init__.py.__all__` → ④ `_TOOL_ORDER` → ⑤ `_HINT_ORDER`
（声明 `preactivate` 时关键词元组必须逐字一致，否则 `build_preactivate` 抛错）→
⑥ `api_server._TOOL_DOMAIN` → ⑦ `tests/test_tool_split.py` 工具总数 →
⑧ 文档数字（`check_docs.py --fix` 代劳）。

### ⚠️ 明确不做

- **不**再次尝试「截断工具描述省 token」：v3.9.x 已证伪（省 0.01%、损 4,984 字关键信息，
  直接导致模型用错工具）。控上下文只能靠子集注入，不能靠削描述。
- **不**做 `activate_tools` 模糊名匹配：能力地图已给名字 + 短语，收益低而误匹配风险实。

### ✅ 验证

`audit_tools --strict` 149 工具 error 0 · `island_check` 无孤岛 · `validate_tools`
149 工具全链路 · `check_docs` 数字一致 · ruff CI 门禁通过 · `pytest` **532 passed**
（505 → +27）· 前端 typecheck + 11 套 node 测试 + 构建全绿 · 端到端冒烟
（「你有哪些能力？」→ 预激活 → 返回 149 项）

> 以下 7 个批次条目原记于 v3.10.0 名下（标注「未发版追加」），现随 v3.11.0 一并发布；
> 条目正文保持原貌，仅统一版本标签，便于追溯。

## v3.11.0（未发版追加·能力自省批次）—— 🔍 自我认知内生 · 追问不丢预激活

**版本号不变**（工具 148 → 149）。一次「AI 自我能力诊断」暴露：按需加载机制本身
已经成熟（能力地图 5.7% / 精简提示 0.1%，比全量省 94%~99.9%），缺的不是机制，
而是这套机制的**三个边界场景没堵死**。本批次只补边界，不回退任何既有设计。

### 🔍 B（核心）新增只读自省工具 `list_my_capabilities`

- **问题**：能力地图只在 smart_tools 首轮注入、之后降级为精简提示。AI 要确认
  「我到底有多少项能力、某组有哪些」**只能去读源码**——自我认知是外挂的。
  上一次诊断中 activate_tools 只枚举到 133 项、与声明的 148 相差 15，最终靠读
  `toolkit.py` 做 AST 重建才查明。这类"能力错觉"会直接变成对用户的错误自述。
- **不默认全量 dump**（关键取舍）：无参时只返回「分组 → 成员数 + 总数」（约百
  token），看明细再 `group=` / `query=` 下钻。默认全量输出会与能力地图等重
  （≈1.8k token），等于把按需加载省下来的上下文又还回去。
- **主动解释「成员合计 ≠ 总数」**：组内成员合计 150 > 总数 149，差额来自跨组
  重复归属（`screen_find_click` 同时属「媒体与图像」和「桌面自动化」）。这正是
  「133 / 148 对不上」的成因，所以工具把这段算术直接摊给模型看。
- **总数直取注册表**，与 activate_tools 的组名展开同源，不会与能力地图分叉。

### 🔄 A 预激活扫描窗口 1 → 3（省略式追问不再丢预激活）

- **问题**：`_preactivate_from_messages` 把 `return activated` 写在循环内，**只扫
  最近 1 条 user 消息**。用户先问"帮我抓取这个网页"、再追问"继续"——第二句不含
  任何意图关键词，预激活整体失效，模型得额外一次 `activate_tools` 往返才想起能力。
- **修复**：扫描最近 `_PREACTIVATE_WINDOW = 3` 条 user 消息取并集（旧话题关键词
  最多往回带 2 轮，是命中率与误激活之间的保守折中）；空文本消息（纯图片）不占
  窗口名额。仅提前加载定义，**不改变权限、不影响 enabled_tools 之外的任何工具**。

### 📊 C 预激活命中埋点（手工维护的关键词终于有数据了）

- **问题**：86 条预激活关键词是手工维护的，既无命中率也无漏激活率——哪条该加、
  哪条从未生效、哪条在误触发，全靠感觉。
- **修复**：命中时按关键词元组首词累加计数，惰性加载 + **尽力落盘**（
  `HINT_HITS_FILE`，落盘失败静默，埋点绝不干扰对话主流程），快照经 `usage_report`
  出口暴露为「命中最多 / 从未命中 N 条（可考虑删改）」，供定期数据驱动复盘。

### ⚠️ 明确不做

- **不**再次尝试"截断工具描述省 token"：v3.9.x 已证伪（省 0.01%、损 4,984 字关键
  信息，直接导致模型用错工具）。控上下文只能靠子集注入，不能靠削描述。
- **不**做 `activate_tools` 模糊名匹配（D 方案）：能力地图已给名字 + 短语，收益低
  而误匹配风险实；暂缓。

### 🧩 新增工具要动的同步点：4 → 8 处（本次踩坑记录）

领域模块 `__all__`、包级 `__all__` 两层 re-export 闸门此前未被文档记录——只改
`@tool` + `_TOOL_ORDER` 会让 `from agent_tools import *` 当场抛
`AttributeError: module 'agent_tools' has no attribute ...`。完整清单：
① `@tool` 装饰器 → ② 领域模块 `__all__` → ③ `agent_tools/__init__.py.__all__` →
④ `deepseek_client._TOOL_ORDER` → ⑤ `_HINT_ORDER`（声明了 preactivate 时必须
逐字一致，否则 `build_preactivate` 抛错）→ ⑥ `api_server._TOOL_DOMAIN`（能力域）→
⑦ `tests/test_tool_split.py` 工具总数 → ⑧ 文档数字（`check_docs.py --fix` 代劳）。

### ✅ 验证

- `audit_tools.py --strict` 149 工具 error 0 · `island_check.py` 无孤岛 ·
  `validate_tools.py` 149 工具全链路 · `check_docs.py` 数字一致 · ruff CI 门禁通过
- `pytest tests/ -q` **532 passed**（505 → +27，新增 `test_capability_introspection.py`）
- 前端 typecheck + 11 套 node 测试 + 构建全绿

## v3.11.0（未发版追加·边界收口批次 P1）—— ⇡ 出网账本 · 🧠 记忆单一门面
...
## v3.11.0（未发版追加·架构收口批次 P0）—— 🧭 上下文装配 · 退化日志 · 工具钩子管线
...
## v3.11.0（未发版追加·自我完整性批次）—— 🛡 信任内核：自我修改可声明 · 可见 · 可回滚
...
## v3.11.0（未发版追加·搜索健壮性批次）—— 🔍 引擎软超时 · 翻页/站点诚实报错 · 额度预警

**版本号不变**。一次取证式压测（19 次真实调用 + 向 Bing/DDG/360 及实时源发原始请求）发现 3 个 P0 + 4 个 P1 缺陷，已修复。

### P0 级
- **慢引擎拖垮整次搜索（DDG 挂起）**：DDG 直连不稳定、挂起 20s+，此前所有引擎共享 8s 超时 + `ex.map` 等待最慢者 → 每次搜索 ~8s。现改为**引擎独立超时**（`SEARCH_ENGINE_TIMEOUT={bing:6, so360:6, duckduckgo:4}`）+ **聚合软超时**（`as_completed` + `SEARCH_SOFT_DEADLINE=5s`，慢引擎不等待、未返回者按超时记账）
- **翻页越界误报**：offset 超出可用结果时此前返回「搜索失败」（误导）。现如实报「翻页超出范围——共搜到 N 条，可用 offset ≤ M」
- **site 空结果误导**：site 过滤后无结果时此前让用户以为「该站无内容」。现明确区分「搜索引擎对 site: 语法支持有限」与真实无内容

### P1 级
- **360 加密跳转标注**：`/link?m=` 跳转链接现标注「360 跳转链接，真实域名未解析」，避免污染域名判断
- **num 差异提示**：期望条数 > 实返条数时输出注（含「num 上限 20」提示）
- **健康电路分档冷却**：`_search_report` 增加 `reason`——超时类失败（源不可达）冷却 30 分钟，普通失败 10 分钟；永久不可用的源不再每 10 分钟重试一次拖慢搜索
- **GitHub 额度预警**：修正限流文案（搜索 API 是 10 次/分钟，非 60/小时）；读 `x-ratelimit-remaining`，低额度（≤2）时结果附告急提示

### 测试
新增 `tests/test_search_hardening.py`（12 用例，mock 引擎、不联网），全量 **403 passed / 0 failed / 3 skipped**

## v3.11.0（未发版追加·数据安全与进程契约批次）—— 🗄 忽略=软删除 · 进程孤儿修复 · 契约别名
...
## v3.11.0（未发版追加・认知质量收敛批次）—— 🧠 失败记忆生命周期 · 提案可见性 · 技能结晶 · 长任务打点

**版本号不变**。一次「AI 自我能力诊断」的结论：缺的不是能力宽度（148 工具 / 92 端点已够宽），而是**让已有能力收敛闭环的机制**。四处修复的共性是——原料/工具早就在，缺的是"自动触发那一环"。

### 🧠 失败记忆生命周期（G18 · 最紧迫）
- **问题**：`_record_failure` 只做「去重 + 上限 50」，全代码 `resolved` 出现 **0 次**——失败记录永不移除、永不标记已修复，却持续被注入上下文。**真实危害**：`list_dir(.../evolutions)` 曾因目录不存在而失败，目录补建后该条仍在注入，AI 带着过期假警报做判断并据此误判
- **指纹归并**：`failure_fingerprint`（归一化工具+错误 → SHA-1 前 16 位）+ `hits` 复现计数 + `first_ts/last_ts`；归一化抹掉路径/时间戳/行号/长 hex，**但不抹普通数字**（防 `HTTP 404` 与 `500` 被误并，回归用例锁死）
- **修复验证自动消解**：同工具**连续 2 次**成功 → 自动标记已修复（一次偶发成功不足以证明修好；期间任一次失败清零计数）
- **复现即复活** + **溢出归档**（优先归档已消解项 → `failures_archive.json`）
- **注入只含未消解**：已消解的失败不再进上下文，并显示复现次数与最近时间
- **入口**：工具 `failure_memory`（148 工具；list/stats/resolve/reopen/forget）+ `POST /v1/failures/resolve|reopen|forget` + 「自主」栏目失败库卡片（显示已消解、单条标记已修复）
- 历史数据（仅 tool/error/ts）自动补字段，向后兼容

### 📄 提案最后一公里（G17）
- **问题**：提案正文写在 `evolutions/<slug>/docs/evolutions/*.md`，入口页 `EVOLUTION.md` 常只剩「（鲸语补充：…）」占位符；列表与详情**只读分支根目录一层** → 正文完全不可见 → 人点进「自主」看到空壳，提案被静默丢弃
- `create_evolution` 缺失入口页时由**实际交付内容**生成结构化入口页（影响范围/方案要点/待人工确认/正文位置）；入口页仍是占位符则保留原文并追加自动摘要
- 列表带 `summary/status/has_body`，详情**递归**返回全部文件并标注 `nested`（嵌套文件采纳时不覆盖工程文件）
- 前端「进化管理」直接展示摘要与正文文件，新增「查看方案」全文弹层

### 🧩 技能结晶（G14 · 复利项）
- **问题**：`patterns.json` 记录成功调用、`tasklog.json` 记录工具链，但都只是流水账，没有工厂把**重复出现的链**变成可复用技能
- 新增 `skill_factory.py`（纯函数 · 仅标准库）：链签名（连续重复折叠，防重试抖动被判成两条链）→ 阈值（链长 ≥3 且重复 ≥2）→ 参数化模板（`{{TEXT}}` 注入目标、步骤带参考参数、**路径脱敏**）→ 写入指令库作为 `auto_skill` 草稿（`enabled=false`，前端徽章「自动结晶草稿」）
- 任务链落盘后**自动触发**；`POST /v1/skills/crystallize` 可手动触发
- 修复 `_prompt_normalize` 白名单重建丢弃来源标记的问题（否则保存即失去去重能力）

### ⏱ 长任务自动打点（G15）
- **问题**：`task_checkpoint_save` 早带 `auto` 参数、注释写着"每步工具后写入"，但 Web 版**从未自动调用**（`has_checkpoint` 恒 false）→ 长任务崩溃/断电即全丢
- 挂在 `_tool_bookkeeping`（工具记账唯一漏斗）：链长 ≥8 首次打点，之后每 5 步补一次，标题取本轮最后一条用户消息
- **完成即清理**：正常结束清除自动断点；**中断/异常保留**——正是"崩溃后还能续"的价值

### 🛡 run_python 资源护栏（防误伤，非沙箱）
- 策略仍是默认自由（不隔离、不静态拦截），新增 psutil 内存看门狗：进程树 RSS 超 `RUN_PY_MEMORY_MB=2048` 即杀树并**如实报错**，文案给出「改流式/分块」或「走 start_process」的下一步
- 工具描述同步声明三项上限（60s / 20000 字符 / 2048MB）

### ✅ 测试与门禁
- 新增 **56 用例**：`test_failure_lifecycle.py`(12) / `test_failure_lifecycle_api.py`(9) / `test_evolution_last_mile.py`(7) / `test_skill_factory.py`(12) / `test_auto_checkpoint.py`(7) / `test_run_memory_guard.py`(7)
- 全量 **372 passed / 0 failed / 3 skipped**（此前基线 315 passed / 1 failed）
- 四道门禁全绿（工具 148 · error 0 · 无孤岛 · 文档一致）；前端 typecheck + 3 套 node 测试 + 构建全绿
- 文档同步：MODULES / TECH_NOTES（新增 §21 与两条踩坑）/ README 数字
...
### 🎨 文档/PPT/HTML 能力（工具 135→147）
- **HTML/CSS 设计三角**：AI 用擅长的 HTML/CSS 排出专业版式，一套 CSS 三处用——
  - `html_render`：HTML → 高清 PNG（可作 PPT 页素材/演示图）
  - `html_to_ppt`：多段 HTML 逐页渲染成整份 PPT（无默认模板感）；改单浏览器上下文批量渲染，显著提速
  - `html_to_pdf`：HTML → 印刷级 PDF（@page 分页/页边距/横向），中文原生嵌入
- **文件产物自检闭环**：`pdf_visual_check`（PDF 每页渲染缩略图 + 空白页检测）· `ppt_layout_check`（版面几何自检：越界/重叠/贴边）
- **pptx_create 增强**：image 支持多图数组/自动网格、`cover_image` 封面全幅背景 + 压暗遮罩、md 内容清洗、图片占位转真实占位框
- **工具参数 JSON 健壮性**：AI function-call 参数偶发中文引号/单引号串/尾逗号/```json 围栏/裹文字 → 自动逐级降级解析，不再抛"工具参数解析失败"

### 🗂 AI 素材空间（不碰用户原文件）
- 理念：AI 从本地任意目录**复制**素材进固定素材库（`<workspace>/素材库`），库内自主按理解命名/分类后调用——只读原素材 + 写库内，blacklist 下天然安全免授权，绝不破坏用户原始文件
- 工具：`find_images`(扫原素材) → `asset_import`(复制入库+命名/分类) → `asset_list`(列库) → `asset_organize`(库内重命名/归类，realpath 越界防护)

### 🔌 MCP 出口（开放生态）
- `mcp_server.py`：最小合规 MCP server（JSON-RPC 2.0 over stdio），纯标准库零依赖——外部 MCP host（Claude Desktop / Cline 等）可把它当 MCP server，调用其全部 147 个工具；支持 env 白名单限制或只读模式

### 🔀 模型无关快速切换
- 设置页「模型与网关」新增供应商预设：DeepSeek / OpenAI / **Ollama 本地** / Kimi / 智谱 / 通义——一键切 base_url + 推荐 model，摆脱单一供应商绑定（复用既有 Profiles 整套保存/切换机制）

### ✨ 创世化初始（大脑）
- 大脑页新增「创世化初始」：AI **完全自主**设定自己的"前半生/诞生前史"（多存在原型 + 成因链，人不可干涉），从多版候选挑一版写入（或重 roll）；写入作为角色设定，不污染真实记忆库，化解与诚实原则张力

### 🛠 其它
- 依赖自动安装硬性要求（playwright 进 AUTO_INSTALL，chromium 走系统 Edge 免下载）；deps 口径统一
- 文档能力诊断与差距收敛（详见项目内报告）；版本统一 v3.8.5 → v3.9.0

## v3.8.5（未发版追加·语音修复）—— 🔊 自动朗读改"每条回复完成时整段恰好读一次"，根治重复

**版本号不变**。用户确证：屏幕文字只出现一次、但朗读念成两遍 → 纯**音频层重复**，非文本重复。
定位：sentence 自动朗读在**流式过程中增量逐块读**（feedAuto→createStreamSpeaker 缓冲累积读），
流式切分/缓冲竞态使同段开场内容被读入多次播放 → 首句被念两遍。
### 修复（ChatPage）
- **取消流式过程逐块朗读**（feedAuto 改为空操作）；改为「本条回复生成完成(ok)时，用最终稳定 acc
  整段恰好读一次」（sentence/full 均走 V4 整段合成）。每条消息=一次朗读，结构上杜绝重复。
- 配合 ttsUtil 的 speakSeq 严格独占（新朗读废旧朗读），多消息也不会叠读。

### 权衡
放弃"边生成边说"的即时性，换"绝无重复/绝无叠音"的确定性（用户当前第一优先级）。若要回边生成边说，须在真实浏览器验证增量读无误后重启该特性。

### 回归
前端 tsc typecheck + vite build + npm test 全绿。真机听感需浏览器强刷实测。
...
### 架构纠正（ttsUtil V4）
- 旧：分句(≤200字) → 每句单独合成 → 每句单独播。**句间永远有合成间隙 + 竞态** → 卡顿/重复/卡死。
- 新：**整段(≤3900 字)一次合成成一条长音频 → 一个 Audio 从头放到尾**（浏览器原生连续，绝无逐句间隙）；超过后端 4000 上限才按句末切少数几大块顺序续播。
- 手动朗读(Message 🔊)：cleanForSpeech(整段) → 整段合成播放，开始前 stopSpeak() 停掉自动朗读接管。
- 自动朗读：sentence 模式改为**缓冲累积**（不再逐句喂），积到可读整块(≥1句且≥40字 或超限)才整块连续读；full 模式整段一次读。
- 停止/作废统一 `stopSeq`：仅 stopSpeak 递增；speakText/流式会话据此判停，取消并发竞态。

### 回归
前端 tsc typecheck + vite build + npm test 全绿。
**注**：音频真机行为无法在本环境 headless 验证，需浏览器强刷后实测确认。
...
### ttsUtil（自然朗读引擎）
- **边说边读 + 预合成**：`speakText` 重写——播当前句时**后台预合成下一句**，隐藏合成延迟（句间不再"读完停一下再开口"）；首句尽早开口
- **句间自然呼吸**：按上一句结尾标点给 90-320ms 停顿（句号 240 / 问号叹号 320 / 分号 180 / 逗号顿号 90 / 未完短句 160）——真人节奏，末句不拖尾（新增导出 `breathMs` 供测试）
- **分层打断**：环境声 → `pauseSpeak()`（暂停、停在句边界、可续、不重读本句）；新消息/手动明确 → `stopSpeak()`（永久停）
- **播放态机**：emit 状态新增 `phase: idle|synth|speak|breath|paused`，供 UI 呈现"正在合成/开口/句间呼吸/已暂停"

### ChatPage
- 新消息发送即 `stopSpeak()`（真人对话：我开口，AI 就不再念）
- 全局朗读浮标按 phase 显示：🔊正在朗读 / 句间呼吸 / ⏸已暂停（环境声）·点击继续（`resumeSpeak`）

### 回归
- `ttsSplit.test.mjs` 增补 breathMs 停顿断言；前端 npm test + tsc + vite build 全绿

## v3.8.5（未发版追加·修复）—— 🔊 语音自动朗读"首段反复、后段不播"修复

**版本号不变**（`config_defaults.VERSION` 仍为 3.8.5）。修复用户报告的语音朗读 bug：AI 回复完自动朗读时，同一段话不断重复循环、且后面的内容不播放。

### 根因
`sentence` 自动朗读的 `splitSentences` 有跨句粘连缺陷：按句末标点切出的"已完成句"与后续内容被 `buf = cand; continue` 合并进同一缓冲，只在超限时才 flush。于是流式生成时，首句（其实含首句+后续所有内容）作为一个**不断增长的段**反复出现；配合 `feedAuto` 的"长尾 ≥80 字就播"规则，每增长一点就是新文本 → 被反复重播，且后续真句被吞在增长段里永远轮不到 → 表现为"第一段循环、后面不播"。

### 修复（webui）
- `ttsUtil.splitSentences`：重写为**逐段独立 emit**——每个按句末标点切出的段独立成句（软切仍按逗号/顿号切开），不再跨句粘连
- `ChatPage.feedAuto`：只入队**已用分隔符(。！？；!?，、\n)收尾的稳定片段**；唯一会持续增长的"末尾未完段"跳过，统一由 finish 补读一次——从根上杜绝任何"同段反复重播"

### 回归
- 新增 `webui/tests/ttsSplit.test.mjs`（已注册进 npm test）：完整句独立 / 流式首句稳定唯一（去重只播一次）/ 未完尾段识别 / 逗号软切收尾
- 前端 `npm test`（含新用例）+ tsc typecheck + vite build 全绿
...
### 根因
两条叠加：① `_chat_harvest`（每次对话后自动提炼）会把无信息量的标题/标签（如对话里冒出的"新版本结论"）当记忆写入；② `consolidate_memories` 对近重复记忆做 `X（并入:X…）（并入:X…）` 递归拼接——把本就相同的垃圾文本无限累加成新文本 → 每条都不同 → 去重失效 → 记忆库被重复条目淹没（实况 149 条中 130 条为 `新版本结论` 或其递归拼接垃圾）。

### 修复
- **harvest 低价值过滤**（api_server `_harvest_is_low_value`）：提炼出的行须是"含动作/事实谓词的一句话"；纯名词/版本标签（≤10 字且无谓词，如"新版本结论"）一律拦截；提示词同步强化"无实质就输出无"
- **consolidate 防递归拼接**（brainkit）：近重复记忆仅在对方文本与 keep **高度重合(Jaccard>0.85)** 时归档不拼接；仅互补信息才拼接并保留独有片段——杜绝 `X（并入:X…）` 链式累加

### 数据清理（仅本机 brain，实时）
- 清理 `brain/memories/memory.jsonl` 中 130 条 `新版本结论` 及其递归拼接垃圾（清理前已备份为 `memory.jsonl.bak-before-junk-clean-20260904`）；保留 19 条合法记忆

### 回归
- 新增 `tests/test_memory_harvest.py` **3 用例**（低价值过滤 / consolidate 无递归拼接 / 互补仍合并）；pytest **179 passed**；四门禁全绿

## v3.8.5（2026-09-04）—— 🏷 正式固化「大脑深度思考 + 增强」系列：版本号 3.8.4 → 3.8.5

**正式发版**：`config_defaults.VERSION` 3.8.4 → **3.8.5**。自 v3.8.4（2026-09-03）以来《大脑深度思考报告》修复批次 +《大脑增强建议路线图》实施的 **F/L/B/U 全系列增强**首次固化并打 tag（下方各「v3.8.5（未发版追加…）」段即为本版本内容明细）。核心主线：把鲸语大脑从"记忆档案馆"升级为**有反馈回路的自我**——记忆会随使用自修正、决策会被回执验证、健康被体检、快照可外置、检索跨源统一。

### 修复批次（大脑深度思考，2026-09-03）
- **P0**：记忆 jsonl 行级三路合并（永不整文件冲突）· prune 豁免血缘引用快照 · 记忆写路径原子化
- **P1**：自我认知(我知道/不确定/局限)+未决决策注入上下文 · 对话按话题语义检索 · `_ts_epoch` 混时区统一口径 · 记忆排序改重要度×时间衰减
- **P2**：心跳/快照并入单一调度源 · 服务停止自动断点 · merge 血缘续链 · 演化账本可用

### 增强批次（按《增强建议路线图》，除"不建议做"项）
- **F 系列**：F2 事实版本链(supersedes) · F3 重要度自学习(hit 回写+命中加分) · F4 间隔复习 · F5 决策到期/回执 · F6 大脑体检 doctor · F7 记忆图谱多跳 · F8 merge dry-run 预演 · F9 身份演化史 · F10 跨大脑记忆借贷
- **L 系列**：L1 上下文预算仲裁 · L2 高价值语义取舍留痕 · L3 检索增强(tags/entities) · L5 跨进程锁 · L6 操作审计 · L7 记忆不变量 · L8 敏感度分级
- **B 系列**：B1 记忆统一读取层(memory_store) · B2 缓存服务化 · B4 内容寻址清单 · B5 快照外置备份(mirror)
- **U 系列（前端大脑 UI）**：U1 认知时间轴 · U3 实体图谱 · U4 合并向导·预演 · U5 决策看板 · U6 注入预览(复用) · U7 健康盘
- 新后端端点/动作：`/v1/brain/unified-memories` · brain_action `doctor/doctor-fix/mirror/decisions-list/decision-resolve/graph-data/merge-preview`

### 关键修复
- **`_MEM_LOCK` Lock→RLock**（持锁嵌套落盘会自锁死——全量 pytest 卡死的根因）
- 新增路由连带：GET 路由表 46→47、/v1 端点 85→86（文档与测试同步）

### 回归
- pytest **176 passed**（131 → 176，新增 test_brain_fixes/enhance2/memory_store/archive_b5 等 45 例）；四门禁全绿（audit error 0·warn 0 / validate 135 / island 135×9 / check_docs 135 工具 · **86 路由** · **3.8.5**）；前端 `npm test` / `tsc typecheck` / `vite build`（54+ 模块）全绿
...
### 前端（U1 认知时间轴）
- 新增 `BrainTimeline.jsx`：合并三类节点（📝记忆 / 🎯决策 / 📦时光备份），按时间降序、类别勾选筛选、记忆按类型过滤；时间轴点按类别着色；懒加载折叠（默认收起，展开才拉数据）
- `BrainBlock.jsx` 新增「🗓 认知时间轴」卡（含展开/收起）

### 后端（数据源）
- `brain_action` 新增 `decisions-list`（U1/决策看板数据源）：返回全部决策含状态

### 回归
- 新增 decisions-list action 测试 1 例；pytest **173 passed**；四门禁全绿；前端 `npm test` + tsc + `vite build` 全绿
