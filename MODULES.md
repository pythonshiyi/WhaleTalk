...
本文档描述鲸语 WhaleTalk 当前（v3.16.10，Web 架构）的模块构成与职责边界，供维护、重构与新增功能时定位。与旧 Tkinter 版（main.py）相关的拆分记录已随 Web 重构归档，不再维护。

> 规模口径（`tools/check_docs.py` 实测）：**165 个 Agent 工具**（11 组）· **101 /v1 端点** · 后端 84 个 pytest 文件 / 845 用例 · 前端 20 个 node 套件；源码约 7.2 万行（根目录 3.3 万 + `agent_tools/` 1.8 万 + `webui/src` 2.1 万）。
...
| `tests/` | 自举回归套件（84 个 pytest 文件 / 845 用例）：注册表六层一致性、域模块拆分 re-export、进化闸、失败生命周期、上下文装配、退化日志、出网账本、记忆门面、信任内核、大脑、安全/权限/网络等；`self_evolve` 验证链自动回退跑全量 |
