...
本文档描述鲸语 WhaleTalk 当前（v3.8.3，Web 架构）的模块构成与职责边界，供维护、重构与新增功能时定位。与旧 Tkinter 版（main.py）相关的拆分记录已随 Web 重构归档，不再维护。v3.6/v3.7 未改变模块布局，仅扩展能力（详见更新记录）。
...
| `agent_tools/` | 运行时工具域模块包（P0-1 拆分，主文件累计 −775 行）：`tool_basic.py`（🔧 系统与基础：get_date/get_weather）、`tool_data.py`（📊 数据与文档：read_csv/write_csv）、`tool_media.py`（🎨 媒体与图像 10 工具：image_process/ocr_image/image_understand/screen_capture/screen_see/chart_read/screenshot_to_html/debug_screenshot/scan_read/image_batch，经 re-import 复用主文件保留的视觉闭环辅助 `_capture_screen_png`/`_extract_image_path`）；各模块顶层 `@tool()` 注册，`__init__.py` 聚合 re-export；deepseek_client 在共享基建后 `from agent_tools import *` 触发注册并保持旧访问路径；门禁工具（audit/validate/island）与 `WhaleTalk.spec`（`collect_submodules`）已多文件适配 |
